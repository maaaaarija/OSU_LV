...
Potrebno je izgraditi i vrednovati model koji procjenjuje emisiju C02 plinova na temelju ostalih numeriˇckih ulaznih veliˇcina. Detalje oko ovog podatkovnog skupa mogu se prona´ci u 3.
...
plt.figure()
plt.scatter(X_train['Fuel Consumption Comb (L/100km)'], y_train, color='blue', label='Training data', alpha=0.5)
plt.scatter(X_test['Fuel Consumption Comb (L/100km)'], y_test, color='red', label='Testing data', alpha=0.5)
...
plt.figure()
plt.hist(X_train['Fuel Consumption City (L/100km)'], color='green', bins=20)
...
plt.figure()
plt.hist(X_train_s['Fuel Consumption City (L/100km)'], color='red', bins=20)
plt.title('After Scaling (Standardization)')
plt.show()

linearModel = lm.LinearRegression()
linearModel.fit(X_train_s, y_train)

print(f"Intercept (theta_0): {linearModel.intercept_}")
print(f"Coefficients (theta_1...m): {linearModel.coef_}")

y_test_p = linearModel.predict(X_test_s)

plt.figure()
plt.scatter(y_test, y_test_p, color='blue', alpha=0.5)
